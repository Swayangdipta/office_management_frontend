...
const StockDetailsTable = ({ assets, setAssets = f => f }) => {
  ...
  const handleSearchChange = (e) => {
    ...
    const filtered = assets.filter((asset) => {
      return (
        asset.stockType.name.toLowerCase().includes(query.toLowerCase()) ||
          ...
          {filteredAssets.length === 0 ? (
            ...
            filteredAssets.map((asset) => (
              <Table.Row key={asset._id} id={asset._id} className="hover:bg-gray-100">
                <Table.Cell className='px-2 border border-sky-700'>{asset.stockType.name}</Table.Cell>
      ...
      </Table>
      {
        isEditing && (
            <div className='w-screen h-screen fixed top-0 left-0 bg-[#00000080] flex items-center justify-center'>
                <StockDetailsForm assets={assets} setAssets={setAssets} asset={isEditing} type='edit' setIsEditing={setIsEditing} />
            </div>
        )
      }
      {
        isDeleteOpen && (<DeletionAsset setIsConfirmationOpen={setIsDeleteOpen} asset_id={isDeleteOpen} type='stock' />)
      }
    </div>
  );
};
